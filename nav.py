...
class BootstrapRenderer(Visitor):
    ...
    def visit_Navbar(self, node):
        ...
        root = tags.nav(_class='navbar navbar-expand-lg navbar-dark bg-dark') if self.html5 else tags.div(
            role='navigation', _class='navbar')
        if hasattr(node, 'kwargs') and node.kwargs.get('class_'):
        ...
        # collapse button
        if hasattr(node, 'kwargs') and node.kwargs.get('responsive') in (None, True):
    ...
    def visit_View(self, node):
        # print "Node dict {}".format(node.__dict__)
        # print "Node active {}".format(node.active)
        item = tags.a(href=node.get_url(), title=node.text,
                      _class='nav-item nav-link')

        if hasattr(node, 'kwargs') and node.kwargs.get('icon'):
